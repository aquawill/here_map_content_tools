...
partition_folder_path = r"decoded/hrn_here_data__olp-here_rib-2/18541222"
            ...
            if re.match('^{}_.*\.json$'.format(layer), f):
                ...
                with open(hmc_decoded_json_file_path, mode='r', encoding='utf-8') as hmc_json:
                    ...
                    with open(output_geojson_file_path, mode='w', encoding='utf-8') as output_geojson:
                        ...
                        addresses_associated_with_parking_area_list = hmc_json['addressesAssociatedWithParkingAreas']
                        parking_area_feature_list = []
                        parking_area_process_progressbar = ProgressBar(min_value=0, max_value=len(parking_area_list),
                                                                       prefix='{} - processing buildings:'.format(f))
                        ...
                        for parking_area in parking_area_list:
                            ...
                            if parking_area.get('geometry'):
                                ...
                                geometry_level_3_name = list(parking_area['geometry'][geometry_level_2_name].keys())[0]
                        ...
                        for addresses_associated_with_parking_area in addresses_associated_with_parking_area_list:
                            parking_area_ref_index = addresses_associated_with_parking_area.get('parkingAreaRefIndex')
                            ...
                            if parking_area_ref_index:
                                if address_ref_index:
                                    if not parking_area_feature_list[parking_area_ref_index].properties.get(
                                            'address_point_ref'):
                                            parking_area_feature_list[parking_area_ref_index].properties['addressesAssociatedWithParkingAreas'] = []
                                    address_ref_id = address_ref_list[address_ref_index]['identifier']
                                    parking_area_feature_list[parking_area_ref_index].properties['addressesAssociatedWithParkingAreas'].append(
                        ...
                        for places_served_by_parking_area_list in places_served_by_parking_area_list:
                            ...
                            confidence_score = addresses_associated_with_parking_area.get('confidenceScore')
                            if parking_area_ref_index:
                                if not parking_area_feature_list[parking_area_ref_index].properties.get('placesServedByParkingAreas'):
                                    parking_area_feature_list[parking_area_ref_index].properties['placesServedByParkingAreas'] = []
                                place_ref_id = place_ref_list[place_ref_index]['identifier']
                                parking_area_feature_list[parking_area_ref_index].properties['placesServedByParkingAreas'].append(
